...
creatGroup() {
	...
	-c \"conf t \" \
	-c \"router bgp 15\" \
    	-c \"no neighbor internal peer-group\" \
	-c \"no neighbor internal remote-as 15\" \
	-c \"no neighbor internal next-hop-self\"" | ./bash-in.sh $1
}
...
setLocalPeer() {
    echo "vtysh \
        -c \"conf t\" \
	-c \"router bgp 15\" \
        -c \"no neighbor $2 peer-group internal\"" | ./bash-in.sh $1
}
